...
class UserDataController extends AbstractActionController
{
    ...
    public function ajaxSetAvatarAction()
    {
        if (!$this->request->isXmlHttpRequest()) {
            return $this->redirect()->toUrl('/');
        }

        $image = $this->saveImageFromRequest($this->request);
        $data = array();
        $data['user_id'] = $this->getServiceLocator()->get('logged_in_user')->getId();
        $data['avatar_url'] = $image->getFilepath();

        $userData = new \Users\Model\UserData();
        $userData->exchangeArray($data);

        $userDataTable = $this->getServiceLocator()->get('user_data_table');
        $userDataTable->saveUserData($userData);

        $answer = array(
            'status' => 'ok',
            'avatar_url' => $image->getFilepath(),
        );
    ...
    public function saveImageFromRequest($request)
    {
        ...
        //$pathFile = $imageFile->getFilepath();
        //$path = "http://" . $_SERVER['SERVER_NAME'] . "/script/thumb.php?src=$pathFile&w=50&h=50";
        //$type = 'gif';
        //$data = file_put_contents("avatar.png", fopen($path, 'r'));
